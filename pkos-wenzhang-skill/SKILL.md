...
| my-voice.md 个人文风档案 | 模板 [references/my-voice-template.md](references/my-voice-template.md)；实例落 `_PKOS/assets/my-voice.md`（可选，缺省走通用规则） |
| gzh-short-post 短文规则 | style-rules.md 短文节（≤1000 字，骨架 A/B） |

**未吸收**：选题/爆款监控四件套（依赖外部数据 API，违反用户「禁外部 API」铁律）、配图排版三件套（space-* 系，与 gzh-design 职责重叠）。
...
inputs:
  ...
  - name: options
    ...
    schema:
      ...
      length_target: "long | short | <int>"     # long=1500-4000（默认 1500-2200）；short=≤1000；int=自定义上限
...
5. **确认文风** → voice_profile 指向 `_PKOS/assets/my-voice.md` 存在则读取为**硬约束**（冲突时档案优先）；否则按 [references/style-rules.md](references/style-rules.md) 通用规则；
...
8. **机器质检** → `python scripts/article_tools.py lint <article.md> --json <qc-report.json>`；fails>0 定点修改后重跑（≤2 轮，P-05）；
...
13. **交付汇报** → 诊断结论、成稿路径、质检结果（字数/最长段/待补清单）、下一步建议（排版→gzh-design；发布→用户审核流程）。
