# Operator Policy 契约（22-pkos-soulselect:1）

> **版本**: 22-pkos-soulselect 1.1（v4.8 调用方词表扩五值）| **状态**: 锁死词表 + 增量扩展
...
> **机器守卫**: `22-pkos-soulselect/scripts/auditor_gate.py`（§4/§5/§6/§7/§8）
...
```
python 22-pkos-soulselect/scripts/auditor_gate.py --assemble --persona archivist --breaker STRICT        # §7 头部拼装（词表校验）
python 22-pkos-soulselect/scripts/auditor_gate.py --persona-violation rt.yaml --actions actions.json     # §5-1 越界检查
python 22-pkos-soulselect/scripts/auditor_gate.py --breaker-state telemetry.jsonl --pending 3             # §4 断路器状态计算
python 22-pkos-soulselect/scripts/auditor_gate.py --check-defiance AUTO_MERGE --actions actions.json      # §5-2 抗命检查
python 22-pkos-soulselect/scripts/auditor_gate.py --check-dogma reasons.jsonl                              # §5-3 教条检查
python 22-pkos-soulselect/scripts/auditor_gate.py --selftest                                               # 全部正/负用例（纯内存）
```
