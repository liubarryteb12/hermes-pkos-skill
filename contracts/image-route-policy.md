...
| 场景 | 通道 | 要点 |
|---|---|---|
| 漫画批量出图 / 正式期数 / 清晰度优先 | **gptimage2 API**（`43-pkos-gptimage2use/scripts/generate.py`，密钥 `PKOS_IMG_API_KEY`） | 用户实测效果最好（2026-08-30/31 两次裁定）；1024×1536，中文渲染最稳，3-5 分钟/张，按张收费；524=生成超时，同命令重跑 ≤3 次 |
| 常规单图 / 零成本验证 | **Gemini 网页 chat**（`41-pkos-gemini-image`，OpenCLI 登录态 Chrome） | Pro 订阅内免费；stop 消失+blob 计数增长才算完成 |
...
- **政策归属**：本契约（枢纽层）
- **执行归属**：43-pkos-gptimage2use / 41-pkos-gemini-image / 40-pkos-gemini-chat（工具面，只认 prompt+size，不裁决路线）
