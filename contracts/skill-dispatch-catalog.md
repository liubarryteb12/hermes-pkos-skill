...
### 2.6c pkos.suitegen.assemble — 套图生产分支（utility，套图资产层，2026-09-13 准入）

**做什么**：消费 31 六段题词 + 知识库素材，按剧本框架组装 8-22 张衔接图集；管理风格 skill 集（双模式进化）。

| 输入 | 输出 |
|---|---|
| 主题/风格/张数/剧本模板 + 31 六段题词 | 套图剧本 + 逐张题词（衔接）+ 出图清单（可商用） |

**触发**：「出套图」「做一套图集」「套图剧本」「风格进化」「喂素材提升风格」

**边界**：绝不出图（用户手动调 27）；与公众号/漫画/PPT/HTML 平行线。
