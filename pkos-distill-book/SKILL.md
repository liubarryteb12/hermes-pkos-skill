...
去重主键与归位规则 → contracts/entry-schema.md；语义分析/发现表 → pkos-analysis；净化 → pkos-polish；出口 → pkos-router。本 skill 只负责「一本书 → 一组干净章节条目 + 导航」这一件事。
...
- **wikilink 纪律**：Step 4 的「与其他章节关联」只链库内真实存在的笔记，否则留章节号纯文本——pkos-audit 的悬空双链检查会抓。
