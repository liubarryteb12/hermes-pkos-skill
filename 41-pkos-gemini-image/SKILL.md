...
同 40-pkos-gemini-chat（OpenCLI 运行时 + 登录态 Chrome + Bridge 连接）。

# 机器脚本（gemini_image.py）

```bash
python 41-pkos-gemini-image/scripts/gemini_image.py --prompt "灯塔插画, 扁平矢量风" --out-dir "D:/.../pkos-outputs/gemini-images"
python 41-pkos-gemini-image/scripts/gemini_image.py --prompt-file p.txt --timeout 300 --keep-in-downloads
```
