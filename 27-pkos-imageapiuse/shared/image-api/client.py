...
def _build_request(
    ...
    base_endpoint = prov["endpoint"].rstrip("/")
    # quirks.endpoint_suffix 允许 provider 覆盖完整路径后缀（如 /images/generations）
    suffix = prov.get("quirks", {}).get("endpoint_suffix")
    if suffix:
        url = base_endpoint + suffix
    else:
        url = base_endpoint + "/images/generations"
    ...
    # v3.3 联通修复: "auto" 值不下发——网关(new-api)按参数路由通道,
    # 带 quality/background=auto 会打到无 gpt-image-2 通道的上游报 503 model_not_found
    ...
    # provider 级额外 body 字段（如 sensenova 要求显式传 watermark/response_format）
    extra_body = quirks.get("extra_body", {})
    if extra_body:
...
def call_generate(
    ...
    url, body, headers = _build_request(cfg, prompt, size, quality, background, negative_prompt)

    ext = prov.get("defaults", {}).get("output_format", "png") or "png"
    dest = output_dir / f"gen-{hashlib.md5(prompt.encode()).hexdigest()[:8]}.{ext}"
...
def main(argv: list[str] | None = None) -> int:
    import argparse
    ap = argparse.ArgumentParser(description="27-pkos-imageapiuse image-api 调用器（调试）")
    ap.add_argument("--prompt", required=True, help="图像提示词")
    ap.add_argument("--size", default="1024x1024", help="尺寸（宽×高，像素）")
