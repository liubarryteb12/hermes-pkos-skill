...
> **27-pkos-imageapiuse 是 PKOS 的原子出图工具，不是出口层。** 它被 11-pkos-ppt-skill / 12-pkos-comic / 其他 skill 或用户直接调用，只负责"调用 gptimage2 并落盘图片"这一件事。
>
...
outputs:
  primary:
    type: Artifact
    schema: "pkos.imageapiuse-result:1"
...
- **入口兼容**：frontmatter `name: 27-pkos-imageapiuse` 不变，触发语不变
...
网关适配：`shared/image-api/client.py`（复用 11-pkos-ppt-skill 已有实现）。
实操手册：`README.md`。
