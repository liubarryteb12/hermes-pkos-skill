#!/usr/bin/env python3
"""
27-pkos-imageapiuse/scripts/generate.py — gptimage2 图片生成器（gpt-image-2）
...
DEFAULT_OUTPUT_DIR = None  # 由 config.json 中 active_provider 对应的 output_dir 决定，若无则按 provider 自动创建 workspace/{provider}-images/
...
def build_manifest(
    ...
    if result.get("file"):
        try:
            ...
            pass
    manifest = {
        "capability_id": "pkos.imageapiuse",
        "version": "1.0.0",
        "generated_at": now,
        "route_id": route_id,
...
def generate(
    ...
    out_dir = out_dir or DEFAULT_OUTPUT_DIR or (
        _SKILL_DIR.parent / "workspace" / f"{cfg['active_provider']}-images"
    )
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(
        description="27-pkos-imageapiuse: 调用 gptimage2 生成图片",
