...
> **27-pkos-gptimage2use 是 PKOS 的原子出图工具，不是出口层。** 它被 11-pkos-ppt-skill / 12-pkos-comic / 其他 skill 或用户直接调用，只负责"调用 gptimage2 并落盘图片"这一件事。
>
...
- **入口兼容**：frontmatter `name: 27-pkos-gptimage2use` 不变，触发语不变
