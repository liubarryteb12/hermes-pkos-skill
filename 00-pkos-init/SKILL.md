---
name: 00-pkos-init
description: 知识库工作区初始化：确定库路径 + 三种架构模式任选其一（MOC 中枢笔记 / OKF 编号类目 / INDEX 索引目录），生成 _PKOS/ 地基与配置。只建骨架，不动库内已有内容。触发语：「初始化知识库」「设置知识库路径」「换个知识库结构」。**v2 双重身份**：保留 v0 `00-pkos-init`（deprecated）作为兼容入口；新会话请用 `pkos.governance.bootstrap`。
---
...
replaces: ["00-pkos-init"]       # 旧 v0 Capability ID
supersedes: []                # 暂未弃用其他
```
...
- **入口兼容**：本 SKILL.md 仍以 `name: 00-pkos-init` 暴露，v0 调用路径不变
...
- [ ] 本 SKILL.md frontmatter 含 `name: 00-pkos-init`（v0 兼容）+ Capability 身份段含 `capability_id: pkos.governance.bootstrap`
- [ ] registry.json 同时含 `00-pkos-init` (status=deprecated) 与 `pkos.governance.bootstrap` (status=registered)
