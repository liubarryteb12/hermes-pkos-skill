---
name: pkos-intake
description: 入口分拣：扫描 _PKOS/INBOX/ 与现有落点，识别每件投放物是什么（PDF/docx/Markdown 等格式、文本层、成品或资料），产出机读分拣单并调度对应规则卡。只做识别、分类、调度——不做内容抽取转换，不改写任何文件。触发语：「处理 INBOX」「分拣一下」「我丢了几个文件」。
---
