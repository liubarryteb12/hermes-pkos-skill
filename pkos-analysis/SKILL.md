---
name: pkos-analysis
description: 对已入库条目做结构化理解：一句话概述+核心要点提取、发现表、可复用性判定（这条能不能复用、怎么复用）、跨域关联推荐，回写状态机。只做理解与关联——不改写原文、不定输出形式。触发语：「分析这篇」「提炼一下要点」「这篇和库里什么有关」。**v2 双重身份**：保留 v0 `pkos-analysis`（deprecated）兼容入口；新会话用 `pkos.analysis.structure`。
---
...
outputs:
  ...
  secondary:
    - kind: "front_matter_append"
      target: "<源条目>"
      fields_added: ["pkos-analysis", "status=analyzed"]
      # v0 行为锁死：仅追加不覆盖；多轮分析累加 pkos-analysis 列表
  side_effects:
...
verification:
  success_predicate:
    ...
    - "源条目 front matter 追加了 pkos-analysis 与 status=analyzed"
...
5. **写回防护（B5 强化）**：源条目 front matter 仅追加 `pkos-analysis`（多轮累加成 YAML 列表）+ 追加/更新 `status=analyzed`；其余字段与正文零改动；用 diff 证明（integrity_policy 门 1 强约束）。

# 回写规范（v0 + v2）

- 源条目 front matter **追加**：`pkos-analysis: "[[新 AN 名]]"`（若已有则为列表追加）、`status: analyzed`。
- **多次分析**：累加成列表而非覆盖——v0 未明示，v2 明确化（避免 B5 误覆盖）。
- 其余字段与正文零改动；用 diff 证明。

# 与 v0 兼容性

- **入口兼容**：frontmatter `name: pkos-analysis` 不变，v0 触发语不变
