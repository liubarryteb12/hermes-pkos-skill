...
# 自主轮次
python pkos-ppt-skill/scripts/compose.py --route _PKOS/routes/RT-XXX.yaml --auto
# 带 AI 插图
python pkos-ppt-skill/scripts/compose.py --route _PKOS/routes/RT-XXX.yaml --ratio 16:9 --images
# 只出 design_spec 调试
python pkos-ppt-skill/scripts/compose.py --route _PKOS/routes/RT-XXX.yaml --ratio 16:9 --dry-run
# 手改 spec 后单独重渲染
python -c "import sys; sys.path.insert(0,'pkos-ppt-skill/scripts'); import json,build_pptx; from pathlib import Path; spec=json.load(open('<...>/design_spec.json',encoding='utf-8')); th=json.load(open('pkos-ppt-skill/themes/aesthetics.json',encoding='utf-8-sig'))['themes'][spec['theme']]['native']; print(build_pptx.build(spec,th,Path('<out.pptx>')))"
```
