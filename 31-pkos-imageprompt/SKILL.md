...
## M05 性张力系列 · 擦边 DNA v2（09-14 语料学习固化）

> 适用 M05 全系列**后续新增条目**；现有 1538 条保持不动（用户裁定勉强能用）。
> 完整版：`workspace/modelscope-prompts/learning/M05-DNA-v2.md`（含槽位池与组装公式）。

**核心认知**：擦边 = 遮与露的博弈，经营「视线被引到哪、什么若隐若现」，不是「露多少」。直白暴露既过不了审也没有张力——欲望来自想象空间。

**八律**（每条正向至少落实 3 律）：
1. 布料半透律——腰腹/胸口半透明薄纱，「若隐若现/隔着薄纱可见」，写透光度不写看到什么
2. 视线引导律——珍珠链/腰链/水珠沿身体曲线「自然下垂、真实重力」，道具与光把视线引到路径上
3. 深V结构律——深V/镂空/露背写成「剪裁结构」（挂脖/绑带/褶皱形成、边缘镶嵌收口），不写成暴露状态
4. S形曲线律——曲线写整体姿态（「一肩略低成 S 形线条」「背沟与腰窝尽收」），不写部位尺寸
5. 局部构图律——景别即欲望取景器：「中近景至大腿构图」「略高于胸部轻微俯拍」
6. 光的位置律——逆光落点清单：发丝边缘/肩膀/锁骨/腰线/布料褶皱，「高光顺着腰臀线流淌」
7. 真实质感律——realistic translucent skin、保留毛孔、不磨皮；真实感放大想象空间
8. 慵懒高贵律——媚态在松弛与克制之间（「高贵且慵懒」「清冷却直视镜头」），反差比搔首弄姿有效

**槽位池**：`learning/m05-allure-pools.json`（眼神14/姿态20/细节20，全过 S00 硬红线扫描）。
**黄金样例**：jackzhang 宫廷珍珠礼服帖（vault _X帖子收藏/Post by @jackzhang123vip on X 2.md）——全篇无直白词，深V+半透薄纱+珍珠垂身+逆光锁骨，每段都在引导视线。
