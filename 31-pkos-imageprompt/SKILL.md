...
3. **逐张扩写成品**：每张产五键 `{code, positive, negative, anchor, params}`；**`positive` 必须是一段 ≥120 字的完整可粘贴提示词（见「成品标准」），不许交付 60 字速写**——把系列锚的光影/色温/质感/镜头语言织进正文，再叠该张专属的主体/动作/道具/机位/细节层次；`anchor` 字段重复系列锚（防漂移的机器抓手，出图时逐张带上）；`negative` 针对该张特有风险（多人场景防串人、夜景防噪点）。
...
# 成品标准（v1.4.0 · 09-14 用户裁定）

> 用户裁定：交付的必须是「精致成品」，不是速写骨架。**题词库里每一条目 = 一段完整可粘贴的正向提示词。**

- **长度**：≥120 字（人像类 180-300 字为宜），单段散文，整段粘进正向框即可出图
- **必含五要素**：① 画幅数字（与 Uxx/C00 一致）② 光线（方向+性质：窗光斜射 / high-key / 直闪 / 单一光源）③ 镜头与构图语言（mm 焦段、机位、景深、虚化、三层纵深）④ 质感与材质（皮肤/织物/介质 + 英文质感词）⑤ 情绪基调
- **校验分层（v1.4.1）**：①②③ 是**硬门禁**（可机械判定：画幅数字 / 光线词 / 镜头构图词 / 长度）；④⑤ 是**软提示**——质感与情绪无法机械判定，`qa_check.py` 只报不拦。**禁止为了过检查硬塞「质感」「情绪」字面词**：字面词门禁曾反向逼出模板化产物（320/320 条以「情绪X、Y」收尾）
- **禁模板化收尾**：同一抽象短语不得跨条复用。每条尾句应补一个正文里没有的**具体细节**（一个动作的余势、一件道具的状态、一处光斑的位置），而不是复述前文
- **美姿四公式（v1.5.0 · 学习成果，源自语料「转/弯/顺/露」美姿法）**：人物条目的姿态句至少落实一处——**转**（头/上身/腿三面只动一个：侧身站立脸转向镜头）、**弯**（一处微弯破直线：头微歪、屈膝、重心偏一侧）、**顺**（线条顺接：手腕放松手背顺接前臂、脚尖顺势延伸）、**露**（手有明确落点：手轻搭大腿外侧、手指自然可见）。四条是调整入口，不全部塞入
- **人物三维度（源自 OpenAI 官方 GPT Image 2.5 指南法则 4）**：人物条目尽量锁定 身体构图（全身可见/包含双脚）+ 视线朝向（看向什么/谁）+ 与环境的真实交互（捧/扶/靠/撑某物），避免假人感
- **岗位分工不内卷（源自语料「三词出神图」拆解）**：摄影语言/版式秩序/概念用途/身份各占自己的短语，不互相堆叠同一维度；材质词不混用逻辑相反的组合（clean lineart ≠ rough lineart）
- **成套叙事（套图脚本用）**：系列条目显式「公共层不变（光/镜头/色盘）+ 本张差异=X（站位/动作/表情/服装状态）」；光源同源铁律——相机只改变观看位置，不搬动太阳
- **学习底稿**：语料提炼细节见 `workspace/modelscope-prompts/learning/STUDY-REPORT.md`（结构模式 11 / 语汇 47 / 叙事法 8 / 反模式 10，全部带出处可溯源）
- **人像类另必含**：「成年」锚点 + 视觉年龄段（S00 硬性要求）
- **英文质感词保留原词**：`Fujicolor Pro 400H tone` / `soft diffused natural sunlight` / `cinematic warm low-key` / `85mm, shallow depth of field` 等——它们是出图质量的抓手，不翻译
- **锚点内化**：每条自带其所属 Fxx 风格锚点的视觉语言（光影/色温/质感）；文件头的「风格锚点」段仍保留，作为同系列跨图一致性的契约
- **反例（不合格）**：`年轻成年女性坐在窗边木桌前，双手捧着陶瓷杯，视线望向窗外，米白针织开衫，桌面有书与眼镜，晨光在侧脸形成柔和高光，背景纱帘微透。` —— 这是素材速写，不是成品

# 单图 / 分格判定（v1.4.0 · 09-14 用户裁定）

> 用户裁定：该出单图就出单图，不许无脑排多格；确实要分格的必须标出来。

**默认单图。** 只有以下情形才分格，且条目内必须标注格数与布局：

| 情形 | 标记写法 |
|---|---|
| 转绘分屏（F08，上下严格 1:1） | `[2格 上下1:1]` |
| 角色设定图 / 三视图（F06 turnaround） | `[2格 正背并排]` |
| 漫画 / 连环叙事 | `[N格 M×N]` |
| 多画面拼贴海报 | `[N格 布局]` |

**单图铁律**：写真、头像、单幅海报、静物、风景、单角色立绘 **一律单图**，不得擅自排成多格或加分割线。判定结果要写进交付说明，让人一眼看出这张是单图还是几格。
...
└── M0x-S0x-X0x-*.md       # 各子题·系列题词正文（现存全为 X01；每支风格首段=风格锚点，条目=完整可粘贴成品提示词）
```
...
6. **成品不缩水**：交付的 positive 必须过「成品标准」硬门禁（长度 + ①②③）；60 字速写不算交付。④⑤ 为软提示，但不得用抽象「情绪」词凑数
7. **单图优先**：默认单图，分格必须在条目里标注格数与布局（见「单图 / 分格判定」）
