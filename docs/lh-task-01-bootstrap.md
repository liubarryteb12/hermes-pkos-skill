...
### 目标
基于已批准的《PKOS 架构演进与能力抽象设计规范 v2.0 Final》文档(路径 `D:\deepseekharness\workspace\skills\personal-knowledge-os\docs\pkos-v2-design.md`)，对 `pkos-init` 这一个 Capability 做机械迁移至 v2 契约规范。

### 本轮范围（仅 1 个 Capability）
- **id 映射**: `pkos-init` → `pkos.governance.bootstrap`
- **阶段**: governance
- **位置**: `D:\deepseekharness\workspace\skills\personal-knowledge-os\pkos-init\`

### 硬约束（违反即重构失败）
1. 不得修改 D-1 ~ D-8 公理
2. 不得修改 C-1 ~ C-6 契约字段
3. 不得删除 v0 Capability（保留为 deprecated alias）
4. 不得重命名 v0 目录（`pkos-init/` 保持）
5. 不得修改 `_PKOS/knowledge/objects/` 已存在的 KnowledgeObject
6. 行为契约（POL/RT 路径、文件命名规则、frontmatter 字段语义）严格保留

### 本轮可改范围
1. **重写 `pkos-init/SKILL.md`**：按 C-1 ~ C-6 契约模板升级，正文增加以下声明段：
...
2. **更新 `pipeline/registry.json`**：
   - 保留 `pkos-init` 条目，标记 `status: "deprecated"`，加 `alias: "pkos.governance.bootstrap"`
   - 新增 `pkos.governance.bootstrap` 条目，stage=governance，required=false，status=registered
   - **不得删除** `pkos-init` 旧条目
3. **新增 `pkos-init/tests/capabilities/pkos.governance.bootstrap.test.yaml`**：每种三态（not_found/ambiguous/unavailable）至少 1 个测试用例
...
### 验收（每条必过）
- [ ] `pkos-init/SKILL.md` 包含 6 段 C-1~C-6 契约
- [ ] `registry.json` 同时含 `pkos-init` (deprecated) 和 `pkos.governance.bootstrap` (registered) 两条
- [ ] `pkos-init/tests/capabilities/pkos.governance.bootstrap.test.yaml` 含 3 个失败三态用例
