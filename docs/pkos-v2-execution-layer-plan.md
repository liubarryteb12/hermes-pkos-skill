...
| 20-pkos-audit / 19-pkos-timeline 未 v2 契约化 | 仍是 v0 SKILL.md，与 pkos.audit.lint 并存 | 契约覆盖不一致 |
...
| 0.3 | 20-pkos-audit v2 化 | `20-pkos-audit/SKILL.md` 加 C-1~C-6 + 失败三态契约（不重写 v0 行为）；registry 新增 `pkos.governance.audit` 条目 + 20-pkos-audit 标 deprecated；**audit.lint 关系**：audit.lint 是 audit 的"扩展执行器"（supersede 关系不变） |
| 0.4 | 19-pkos-timeline v2 化 | `19-pkos-timeline/SKILL.md` 加 C-1~C-6 + 失败三态契约；registry 新增 `pkos.maintenance.timeline` + 19-pkos-timeline 标 deprecated |
...
| 能力 | 脚本 | 入口 | 关键设计 |
|---|---|---|---|
| `pkos.maintenance.index` | `16-pkos-maintenance-index/scripts/index.py` | `python -m scripts.index <vault> --out _PKOS/` | 扫库解析 front matter → MASTER_INDEX.md + .json；支持 incremental |
...
| `pkos.audit.lint` | `17-pkos-audit-lint/scripts/lint.py` | `python -m scripts.lint <vault> --mode auto-fix --rules fm-missing,status-machine,tag-coverage` | 4 类可修 + 4 类只报告（dangling/orphan/cross-domain/status-retrograde-blocked） |
...
- `18-pkos-fanout-concept/SKILL.md`：新增 **"执行提示"** 段——host/agent 会话内如何按 6 扇出方向调 LLM（按 provider-policy）、如何读 MASTER_INDEX.json 找关联、如何保证扇出草稿不自动 commit
...
- [ ] 0.3 20-pkos-audit v2 化
- [ ] 0.4 19-pkos-timeline v2 化
- [ ] 0.5 .staging/ 归档
...
- `21-pkos-meta`（infra_non_units，无 SKILL.md）—— 是否补文档？**暂不做**（元治理设施结构上属"非能力"类）
- `19-pkos-timeline/SKILL.md` 与 `pkos.audit.lint` 的关系——audit.lint 产报告 / timeline 读报告，**未明确数据契约**，等梯次 2 e2e 验证
