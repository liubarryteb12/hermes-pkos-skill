...
| pkos-comic | `pkos.exit.comic.compose` | exit | `scripts/compose.py`（+ extract_characters / render_script / validate_manifest / event_bus） | 漫画脚本/分镜出口 |
| pkos-gzhxiaoshuo-skill | `pkos.exit.gzhxiaoshuo.compose` | exit | `scripts/gzhxiaoshuo_tools.py` | 公众号小说/长文出口 |
| pkos-wenzhang-skill | `pkos.exit.wenzhang.compose` | exit | `scripts/article_tools.py`（lint/stats/selftest） | 公众号文章出口 v1.0：六写法诊断路由 + 成稿+标题矩阵+机器质检（吸收 creator-buddy） |
...
| pkos-meta | （治理面，无 capability_id） | meta | `scripts/tick.py`、`telemetry_dashboard.py`、`meta_gate.py` | 心跳巡检 / 遥测面板 / meta 门禁 |
