...
- **adapter 只做 prompt → 该渠道 API 的格式转换**——把 pkos-comic 产出的 prompt 块,转成 Nano Banana / gptimage2 / 可灵 等能直接调用的请求体
...
- 不做 prompt 优化/翻译(出图 prompt 已由 pkos-comic 拼装好,adapter 只做格式转换)
