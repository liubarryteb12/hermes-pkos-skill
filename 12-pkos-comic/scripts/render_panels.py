#!/usr/bin/env python3
"""12-pkos-comic 出图渲染器 —— 消费 compose 产物（manifest.json），调用 gptimage2 真出图。

补齐 compose.py 缺失的生图环节（prototype 阶段只有脚本+prompts txt，0 张图）。

用法：
  python 12-pkos-comic/scripts/render_panels.py --manifest _PKOS/outputs/RT-xxx-comic-script/manifest.json
  python 12-pkos-comic/scripts/render_panels.py --manifest ... --cover-only
  python 12-pkos-comic/scripts/render_panels.py --manifest ... --no-skip-existing
...
sys.path.insert(0, str(SKILL_ROOT / "11-pkos-ppt-skill" / "shared" / "image-api"))
sys.path.insert(0, str(SKILL_ROOT / "12-pkos-comic" / "scripts"))

import client  # noqa: E402  (11-pkos-ppt-skill/shared/image-api/client.py)
...
def main() -> int:
    ap = argparse.ArgumentParser(description="12-pkos-comic render_panels — 真出图")
