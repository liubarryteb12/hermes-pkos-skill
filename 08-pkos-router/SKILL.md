...
verification:
  success_predicate:
    ...
    - "v4.0: 机器守卫 08-pkos-router/scripts/strategy_gate.py --selftest 全 PASS"
...
- **入口兼容**：frontmatter `name: 08-pkos-router` 不变，v0 触发语不变
...
- schema 严格匹配（机器守卫 `08-pkos-router/scripts/strategy_gate.py --selftest` 全 PASS；LLM 产出建议 jsonschema 校验——当前环境缺该库，缺库时按字段清单人工核对并 fail loud，不静默放过）
- 不在 schema 的字段 → 拒绝
...
`08-pkos-router/scripts/strategy_gate.py`：
