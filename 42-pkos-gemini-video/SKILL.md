...
> **骨架占位单元**：与 40-pkos-gemini-chat / 41-pkos-gemini-image 同族（OpenCLI 外部能力三件套：对话/生图/视频）。接口面已锁死，实现后只填 `gemini_video.py`，不动契约结构。
>
...
```bash
python 42-pkos-gemini-video/scripts/gemini_video.py --prompt "..." --out-dir "D:/.../pkos-outputs/gemini-videos" [--duration 8]
# 产出: <out-dir>/<ts>-<slug>.mp4 + .manifest.json（schema: 42-pkos-gemini-video:1）
```
