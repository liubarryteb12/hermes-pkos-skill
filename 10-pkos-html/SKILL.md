...
> - conversion_type 四值词表的权威定义在 [08-pkos-router SKILL.md](../08-pkos-router/SKILL.md)
>
...
`style_theme` 非空（router 越权）｜`conversion_type` 不在四值词表（权威定义见 [08-pkos-router SKILL.md](../08-pkos-router/SKILL.md)「路由单 schema」：wiki百科条目｜实战操作指南｜避坑风险清单｜学习路径）｜源条目 status 早于 analyzed。
...
- **入口兼容**：frontmatter `name: 10-pkos-html` 不变，v0 触发语不变
